package TSIG_LAB.TSIG_LAB.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "TSIG_LAB-ear.ear");

        // Current directory is the root of the "TSIG_LAB-web" project. Go up one level
        // and enter the "TSIG_LAB-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../TSIG_LAB-ejb/target/TSIG_LAB-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "TSIG_LAB-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/TSIG_LAB-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "TSIG_LAB-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("TSIG_LAB.TSIG_LAB.test");
